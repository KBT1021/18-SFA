...
public class HologramOrigin : MonoBehaviour 
{
    public Camera[] cameras;

    public float hologramAreaOrigin = 1f;
    ...
    void Scale()
    {
        transform.localScale = new Vector3(hologramAreaOrigin, hologramAreaOrigin, hologramAreaOrigin);
    }

    void CameraClipping()
    {
        float centerToEdge = hologramAreaOrigin / 2f;
        float EdgeToCamera = centerToEdge / (float)(Math.Tan(30 * (Math.PI / 180)));

        for (int i = 1; i < cameras.Length; i++)
        {
            cameras[i].farClipPlane = EdgeToCamera + hologramAreaOrigin;
            cameras[i].nearClipPlane = EdgeToCamera/10f;
        }
    }
